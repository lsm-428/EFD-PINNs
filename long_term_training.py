...
def main():
    ...
    if os.path.exists(dataset_path):
        ...
        physics_points = torch.tensor(data['physics_points'], dtype=torch.float32)
    else:
        if args.use_3d_mapping:
            try:
                import importlib.util
                spec = importlib.util.spec_from_file_location('gp3d', os.path.join(os.path.dirname(__file__), 'generate_pyvista_3d.py'))
                mod = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(mod)
                from ewp_parameter_mapper import EWPParameterMapper
                ...
                # GPU安全生成：分块构造
                chunk = 512 if device.type == 'cuda' and args.gpu_safe else args.num_samples
                batches = []
                remaining = args.num_samples
                while remaining > 0:
                    cur = min(chunk, remaining)
                    base_list = mapper.create_stage3_batch_from_3d(cur, mod.material_structure, mod.dimensions, voltage=mod.dimensions.get('电学参数', {}).get('最大工作电压', 30.0)/2)
                    features_array = input_layer.create_batch_input(base_list)
                    batches.append(input_layer.to_tensor(features_array))
                    remaining -= cur
                X_data = torch.cat(batches, dim=0)
                y_data = torch.randn(X_data.shape[0], config['模型']['输出维度'], device=device)
                # 划分集并保存一致
                n = len(X_data)
                train_size = int(n * config['数据']['训练集比例'])
                val_size = int(n * config['数据']['验证集比例'])
                ...
                val_indices = indices[train_size:train_size+val_size]
                test_indices = indices[train_size+val_size:]
                X_train = X_data[train_indices]; y_train = y_data[train_indices]
                X_val = X_data[val_indices]; y_val = y_data[val_indices]
                X_test = X_data[test_indices]; y_test = y_data[test_indices]
                physics_points = X_data[torch.randperm(n, device=device)[:min(1000, n)]]
                dataset_path = os.path.join(args.output_dir, 'dataset.npz')
                ...
                logger.info(f"3D映射数据集已保存到: {dataset_path}")
            except Exception as e:
                logger.error(f"3D映射生成失败，回退到随机数据: {str(e)}")
    ...
    model, loss_history = run_training(
        ...
        device, args, dynamic_weight_integration
    )
