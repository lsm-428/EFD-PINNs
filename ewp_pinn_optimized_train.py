...
def progressive_training(config_path='model_config.json', resume_training=False, resume_checkpoint=None, mixed_precision=True, model_init_seed=None, use_adaptive_hyperopt=False, enable_performance_monitor=True, enable_advanced_regularization=True, use_efficient_architecture=True, model_compression_factor=1.0, output_dir=None):
    """
    ...
    # 初始化模型性能监控器（统一到输出目录/reports）
    performance_monitor = None
    if enable_performance_monitor:
        perf_dir = os.path.join(output_dir, 'reports')
        try:
    ...
    for stage_idx, stage_config in enumerate(training_stages):
        ...
        try:
            ...
            stage_out_dir = os.path.join(output_dir, 'visualizations')
            os.makedirs(stage_out_dir, exist_ok=True)
            with open(os.path.join(reports_dir, f'constraint_diagnostics_stage_{stage_idx+1}.json'), 'w', encoding='utf-8') as f:
                json.dump(rep, f, indent=2, ensure_ascii=False)
            try:
                from scripts.visualize_constraint_report import plot_residual_stats, plot_weight_series
                plot_residual_stats(rep, stage_out_dir)
                plot_weight_series(rep, stage_out_dir)
            except Exception:
                pass
        except Exception:
            pass
    ...
    try:
        ...
        try:
            ...
            plot_residual_stats(rep, viz_dir)
            plot_weight_series(rep, viz_dir)
        except Exception:
            pass
    except Exception:
        pass
    ...
    # 生成并保存训练曲线图
    # 保存统一训练历史与曲线到output_dir
    try:
        import json
        serializable_history = {
            'train_losses': train_history,
            'val_losses': val_history,
            'epochs_completed': sum(stage.get('轮次', 0) for stage in training_stages)
        }
    ...
    try:
        # 将训练曲线复制到统一visualizations目录
        src_curve = os.path.join(save_dir, 'training_curves.png')
        dst_curve = os.path.join(visuals_dir, 'training_curves.png')
    ...
    try:
        ...
        np.savez_compressed(
            ...
            physics_points=(X_phys.cpu().numpy() if X_phys is not None else np.empty((0, config['模型']['输入维度'])))
        )
...
if __name__ == '__main__':
    from efd_pinns_train import main as unified_main
    unified_main()
