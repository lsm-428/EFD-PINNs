...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='增强版EWPINN训练脚本')
    parser.add_argument('--config', type=str, default='./ewp_pinn_configuration.py',
                      help='配置文件路径')
    ...
    parser.add_argument('--deterministic', action='store_true', default=True,
                      help='启用确定性模式以提高可重复性')
    parser.add_argument('--debug_capture_nan', action='store_true', default=False,
                      help='在出现 NaN/Inf 损失时捕获并保存 batch 与模型状态以便调试')
    parser.add_argument('--clip_grad', type=float, default=None,
                      help='可选：在训练时覆盖配置中的梯度裁剪阈值（max_norm）')
    parser.add_argument('--override_lr', type=float, default=None,
                      help='可选：覆盖每阶段创建的优化器学习率')
...
def main():
    ...
    set_global_seed(seed=getattr(args, 'seed', 42), deterministic=getattr(args, 'deterministic', True))
    if getattr(args, 'clean', False) or getattr(args, 'clean_all', False):
        os.makedirs(args.output_dir, exist_ok=True)
        clean_outputs(args.output_dir, clean_all=args.clean_all)
        return 0
    ...
    try:
        os.makedirs(os.path.join(args.output_dir, 'logs'), exist_ok=True)
        file_handler = logging.FileHandler(os.path.join(args.output_dir, 'logs', 'enhanced_training.log'))
    ...
    # 加载配置
    if args.quick_run:
        logger.info("使用极少训练量模式（快速运行）")
        # 优先使用用户指定的配置文件
        if args.config and os.path.exists(args.config):
            logger.info(f"快速运行模式下使用用户指定的配置文件: {args.config}")
            config = load_or_create_config(args)
        else:
            logger.info("快速运行模式下使用默认快速配置")
            config = create_quick_run_config()
        # 自动设置较少的样本数
        args.num_samples = 100
        
        # 现在设置quick_run的特殊目录结构
        # 创建一个quick_run子目录
        quick_run_dir = os.path.join(args.output_dir, 'quick_run')
        os.makedirs(quick_run_dir, exist_ok=True)
        os.makedirs(os.path.join(quick_run_dir, 'checkpoints'), exist_ok=True)
        os.makedirs(os.path.join(quick_run_dir, 'logs'), exist_ok=True)
        os.makedirs(os.path.join(quick_run_dir, 'visualizations'), exist_ok=True)
        
        # 更新output_dir为quick_run目录
        args.output_dir = quick_run_dir
        logger.info(f"快速运行输出目录已设置: {args.output_dir}")
    else:
        config = load_or_create_config(args)
    
    # 生成或加载数据
    dataset_path = os.path.join(args.output_dir, 'dataset.npz')
    
    if os.path.exists(dataset_path) and not args.generate_data_only:
        logger.info(f"加载现有数据集: {dataset_path}")
        ...
        X_train, y_train, X_val, y_val, X_test, y_test, physics_points = generate_training_data(
            config, args.num_samples, device, args.output_dir
        )
    
    # 如果只生成数据，退出
    if args.generate_data_only:
        logger.info("数据生成完成，程序退出")
        return 0
    
    # 初始化模型
    if args.validate_only and args.model_path:
        # 加载预训练模型
    ...
    # 创建标准化器并使用训练数据拟合
    normalizer = DataNormalizer(feature_method='standard', label_method='minmax')
    normalizer.fit(X_train, y_train)
    
    # 验证模型
    if args.validate_only:
        validate_model(model, X_test, y_test, config, device, args.output_dir, 
                      normalizer=normalizer, physics_points=physics_points)
        return 0
    
    # 运行训练
    import copy
    model, training_history = run_enhanced_training(
        model, config, 
    ...
    # 最终验证
    logger.info("\n=== 最终模型验证 ===")
    validation_results = validate_model(model, X_test, y_test, config, device, args.output_dir, 
                                      normalizer=normalizer)
    
    logger.info("\n=== 训练总结 ===")
    logger.info(f"总训练轮次: {training_history['epochs_completed']}")
    logger.info(f"最佳验证损失: {min(training_history['val_losses']):.6f}")
    logger.info(f"最终测试损失: {validation_results['test_loss']:.6f}")
