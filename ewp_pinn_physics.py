...
class PhysicsConstraints:
    ...
    def compute_volume_conservation_residual(self, x_phys: torch.Tensor, predictions: torch.Tensor):
        try:
            ...
            batch = predictions.shape[0] if isinstance(predictions, torch.Tensor) else 1
            # 从输入中近似提取油墨体积分数与厚度分数
            # 这里假设x_phys的最后几维包含归一化特征；无法直接索引时使用0.5占位
            if isinstance(x_phys, torch.Tensor) and x_phys.shape[-1] >= 62:
                # 简化近似：使用倒数第2维作为ink_volume_fraction的代理，倒数第3维作为ink_thickness_norm代理
                ink_vol_frac = x_phys[:, -2]
                ink_thick = x_phys[:, -3]
                polar_thick = 1.0 - ink_thick
            else:
                ink_vol_frac = torch.full((batch,), 0.5, device=device)
                ink_thick = torch.full((batch,), 0.5, device=device)
                polar_thick = 1.0 - ink_thick
            # 体积守恒残差：油墨体积分数与厚度分数的一致性
            vol_consistency = ink_vol_frac - ink_thick
            # 双层总分数守恒：ink_thickness_norm + polar_thickness_norm ≈ 1
            total_fraction = ink_thick + polar_thick - 1.0
            # 返回规范化的残差项
            def normalize(res):
                denom = torch.max(torch.abs(res)) + 1e-12
                return res / denom if denom > 0 else res
            return {
                'volume_conservation': normalize(total_fraction),
                'volume_consistency': normalize(vol_consistency)
            }
        except Exception as e:
            logger.error(f"体积守恒残差计算失败: {str(e)}")
            return {
                'volume_conservation': torch.zeros(predictions.shape[0], device=predictions.device),
                'volume_consistency': torch.zeros(predictions.shape[0], device=predictions.device)
            }
...
class PINNConstraintLayer(nn.Module):
    ...
    def __init__(self, physics_constraints=None, residual_weights=None, 
                 enable_dynamic_weight=True, dynamic_weight_config=None):
