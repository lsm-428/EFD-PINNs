...
class PhysicsConstraints:
    ...
    def compute_navier_stokes_residual(self, x, predictions, model=None):
        ...
        try:
            ...
            # 材料参数 (基于 PHYSICS 常量)
            rho_oil = self.materials_params.get('density_ink', 800.0)
            rho_polar = self.materials_params.get('density_polar', 1000.0)
            mu_oil = self.materials_params.get('viscosity_ink', 0.003)
            mu_polar = self.materials_params.get('viscosity_polar', 0.001)
            sigma = self.materials_params.get('surface_tension_polar_ink', 0.045)
            
            # 电润湿参数
            epsilon_0 = self.materials_params.get('epsilon_0', 8.854e-12)
            epsilon_r = self.materials_params.get('relative_permittivity', 12.0)
            ...
            # f_ew = f_ew_magnitude * phi * ∇phi / |∇phi|
            # 其中 phi 确保力只作用在油墨区域
            # ∇phi/|∇phi| 给出界面法向方向（指向油墨侧）
            ...
            # 电润湿力方向: 从中心向外（使油墨向边缘收缩）
            # 力作用在油墨上，方向为 ∇phi 的反方向（指向油墨外侧）
            center_x, center_y = Lx / 2, Ly / 2
            dx_from_center = x_coord - center_x
            dy_from_center = y_coord - center_y
            dist_from_center = torch.sqrt(dx_from_center**2 + dy_from_center**2 + 1e-12)
            
            # 归一化方向向量（从中心向外）
            dir_x = dx_from_center / dist_from_center
            dir_y = dy_from_center / dist_from_center
            
            # 电润湿体积力（作用在油墨区域，即 phi > 0.5）
            # 力的大小与 V² 成正比，驱动油墨向外收缩
            ink_fraction = phi  # 油墨体积分数
            f_ew_x = f_ew_magnitude * ink_fraction * interface_indicator * z_decay * dir_x
            f_ew_y = f_ew_magnitude * ink_fraction * interface_indicator * z_decay * dir_y
            f_ew_z = torch.zeros_like(f_ew_x)  # z 方向无电润湿力

            # 连续性方程
            continuity = u_x + v_y + w_z

            # 动量方程 (包含对流项 + 表面张力 + 电润湿力)
            u_conv = u * u_x + v * u_y + w * u_z
            v_conv = u * v_x + v * v_y + w * v_z
            w_conv = u * w_x + v * w_y + w * w_z

            # N-S: ρ(∂u/∂t + u·∇u) = -∇p + μ∇²u + f_st + f_ew
            momentum_u = rho * (u_t + u_conv) + p_x - mu * lap_u - f_st_x - f_ew_x
            momentum_v = rho * (v_t + v_conv) + p_y - mu * lap_v - f_st_y - f_ew_y
            momentum_w = rho * (w_t + w_conv) + p_z - mu * lap_w - f_st_z - f_ew_z
            ...
            if isinstance(predictions, torch.Tensor):
                if predictions.shape[1] >= 5:
                    ...
                    if isinstance(x_phys, torch.Tensor) and x_phys.dim() == 2 and x_phys.size(1) >= 3:
                        ...
                        above_ink = z > ink_thickness
                        overflow_mask = near_wall & above_ink
    ...
    def compute_sidewall_contact_angle_residual(self, x_phys: torch.Tensor, predictions: torch.Tensor):
        try:
            device = x_phys.device if isinstance(x_phys, torch.Tensor) else torch.device('cpu')
            batch_size = predictions.shape[0] if isinstance(predictions, torch.Tensor) else 1
            residuals = {
            ...
            near_wall = near_left | near_right | near_front | near_back
            wall_height = self.materials_params.get('wall_height', 20e-6)
            above_wall = z > wall_height
            mask = near_wall & above_wall
            if mask.any():
                penalty = alpha_clamped * mask.float()
                residuals['sidewall_contact_angle'] = penalty
            return residuals
        except Exception as e:
    ...
    def compute_wall_wetting_residual(self, x_phys: torch.Tensor, predictions: torch.Tensor):
        try:
            device = x_phys.device if isinstance(x_phys, torch.Tensor) else torch.device('cpu')
            batch_size = predictions.shape[0] if isinstance(predictions, torch.Tensor) else 1
            residuals = {
                'wall_wetting': torch.zeros(batch_size, device=device, requires_grad=True)
            }
            if not (isinstance(x_phys, torch.Tensor) and isinstance(predictions, torch.Tensor)):
                return residuals
            if x_phys.dim() != 2 or x_phys.size(1) < 3:
    ...
    def compute_core_residuals(
        ...
        try:
            ns_residuals = self.compute_navier_stokes_residual(x_phys, predictions, model=model)
        ...
        # 3. 表面张力残差
        try:
            st_residuals = self.compute_surface_tension_residual(x_phys, predictions)
            residuals.update(st_residuals)
        except Exception as e:
            logger.warning(f"表面张力残差计算失败: {e}")
    ...
    def _compute_vof_residual(
        ...
        try:
            ...
            # 改进方案: 添加人工压缩项 (Artificial Compression Term)
            # ∂φ/∂t + ∇·(uφ) + ∇·(u_c φ(1-φ)) = 0
            # 这里我们使用非守恒形式，所以是 ∂φ/∂t + u·∇φ + ∇·(u_c φ(1-φ)) = 0
            # u_c = C_alpha * |u| * n_interface
            
            # 1. 计算界面法向量 n = ∇φ / |∇φ|
            grad_phi_mag = torch.sqrt(phi_x**2 + phi_y**2 + phi_z**2 + 1e-10)
            n_x = phi_x / grad_phi_mag
            n_y = phi_y / grad_phi_mag
            n_z = phi_z / grad_phi_mag
            
            # 2. 确定压缩速度大小 |u_c|
            # 通常取局部流速的大小，或者一个特征速度
            vel_mag = torch.sqrt(u**2 + v**2 + w**2 + 1e-10)
            c_alpha = 1.0  # 压缩系数，可调节
            
            # 3. 构建压缩通量 flux_c = c_alpha * |u| * φ(1-φ) * n
            # 为了数值稳定性，φ(1-φ) 在 φ=0,1 时为 0，这能自动限制压缩范围
            ...
            # 4. 计算压缩项的散度 ∇·flux_c
            g_flux_x = get_grad(flux_cx.sum(), x_phys)
            g_flux_y = get_grad(flux_cy.sum(), x_phys)
            g_flux_z = get_grad(flux_cz.sum(), x_phys)
            
            compression_term = g_flux_x[:, 0] + g_flux_y[:, 1] + g_flux_z[:, 2]
...
class PINNConstraintLayer(nn.Module):
    ...
    def compute_physics_loss(self, x_phys, model_predictions, data_loss=None, val_loss=None, epoch=None, stage=None, applied_voltage=None, contact_line_velocity=None, time=None, temperature=None, model=None):
        """
        计算物理约束损失
        ...
        if hasattr(self.physics_constraints, 'compute_sidewall_contact_angle_residual'):
            try:
                sw_residuals = self.physics_constraints.compute_sidewall_contact_angle_residual(
                    x_phys, model_predictions
                )
